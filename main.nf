...
process gcta_grm {
    ...
    input:
    set file(vcf), file(index) from vcf_to_whole_gcta_grm

    output:

    """
    bcftools annotate --rename-chrs rename_chromosomes Phenotyped_Strain_VCF.vcf.gz -Oz -o Phenotyped_Strain_RENAMED.vcf.gz
    tabix -p vcf Phenotyped_Strain_RENAMED.vcf.gz

    plink --vcf Phenotyped_Strain_RENAMED.vcf.gz \
    --snps-only \
    --biallelic-only \
    --maf 0.05 \
    --set-missing-var-ids @:# \
