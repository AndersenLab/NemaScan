...
This analysis was run using two algorithms from GCTA: INBRED and LOCO. XXX (Insert text explaining the differences between the algorithms and link to sam's nemascan paper as well as gcta paper for more info)
...
### QQ-plots to help decide between INBRED and LOCO GWA algorithms
XXX (Insert text here to talk about how to interpret the plot)
...
qq_plot <- exp_inbred %>%
  ...
  dplyr::filter(CHROM != "MtDNA") %>%
  ggplot2::ggplot(.) +
  ggplot2::aes(x = -log10(exp.P), y = -log10(P), fill = algorithm, color = significant) +
  ggplot2::geom_abline(slope = 1) +
  ggplot2::geom_point(shape = 21) +
  ggplot2::scale_fill_manual(values = c("LOCO" = "white", "INBRED" = "black")) +
...
The genomic inflation is `r calc.genomic.inflation(exp_inbred, trait_name)$GIF[1]` for the INBRED algorithm and `r calc.genomic.inflation(exp_loco, trait_name)$GIF[1]` for LOCO. XXX (Explain what this means)

The following sections of the report are shown for both the INBRED and LOCO parameters. It is recommended you choose one based on the previous plots. This could be different between different traits.

# INBRED

This is the default parameter. XXX (more here)
...
For some traits, LOCO is better because it ignore structure on the chromosome of the main QTL, allowing for higher power to detect linked QTL. However, this higher power also comes with higher false positive rates. For more info, check out this paper. XXX (more here)
