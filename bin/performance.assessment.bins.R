...
bins <- as.numeric(args[2])
markers <- as.numeric(args[3])
...
summarize.by.bp.bins <- function(x,y){
   bins <- bins
   ## can we do binning by a set number of bp

   x %>%
      dplyr::mutate(bp.bin = cut(POS, bins)) %>%
      dplyr::group_by(bp.bin) %>%
      dplyr::mutate(CHR = y) %>%
      dplyr::summarise(median(POS),
                       ...
                       max(POS),
                       max(POS)-min(POS)) %>%
      `colnames<-`(c("bp.bin","median(POS)","max(log10p)","TRUE.HIT","DETECTED.HIT","median.SNP.effect","bin.median.MAF","marker.density","CHR","start","stop","bin.width.bp"))
}
...
fastGWA.inbred.bp.bin.assessment <- function(x){

   print(paste("Assessing fastGWA (inbred) - BP Bins", x, sep = " "))
   ...
   mapping <- read.table(paste(paste(strsplit(x, split = "_")[[1]][1],
      ...
      dplyr::group_by(CHR) %>%
      tidyr::nest()
   binned.mapping <- purrr::map2(mapping$data, mapping$CHR, summarize.by.bp.bins)
...
simulation.metrics.bp.binned <- purrr::map(iterations, fastGWA.inbred.bp.bin.assessment)
fastGWA.inbred.assessment.bp.binned.dat <- do.call(rbind, simulation.metrics.bp.binned)
...
save(fastGWA.inbred.assessment.bp.binned.dat,
     fastGWA.inbred.assessment.SNP.binned.dat,
     file = paste("Sim.Perf.Bin",bins,"bp",markers,"markers","RData", sep = "."))

# # Marker-Based Performance
# fastGWA.inbred.marker.assessment <- function(x){
#    
#    print(paste("Assessing fastGWA (inbred) - Markers", x, sep = " "))
#    nQTL <- strsplit(x,split = "_")[[1]][1]
#    rep <- strsplit(x,split = "_")[[1]][2]
#    h2 <- strsplit(x,split = "_")[[1]][3]
#    
...
#    
# }
# simulation.metrics <- purrr::map(iterations, fastGWA.inbred.marker.assessment)
# fastGWA.inbred.assessment.markers.dat <- do.call(rbind, simulation.metrics)
# save(fastGWA.inbred.assessment.markers.dat,
#      file = "Simulation.Performance.Markers.RData")
