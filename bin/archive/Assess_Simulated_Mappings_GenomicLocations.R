...
GL.result.regex <- glob2rx("Genomic.Locations*.unswept.*")
GL.result.paths <- list.files(pattern = GL.result.regex)
simulation.metrics <- function(x, result.path){
   ...
   if(is.character(mapping.aggregate$result)){
      ...
      all.QTL <- data.frame(c(effects.scores$QTL, overlap$QTL)) %>%
         ...
         dplyr::mutate(algorithm = "MIXED",
                       ...
                       sim = x,
                       Gen.Loc.Info = paste0("SimChr",paste(strsplit(result.path,split = "[.]")[[1]][c(4,5,7)], collapse = "_")))
...
GL.metrics.list <- list()
# for(i in seq(1:5)){
for(i in 1:length(GL.result.paths)){
   result.path <- GL.result.paths[[i]]
   setwd(paste("/projects/b1059/projects/Sam/NemaScan",result.path,"Simulations",sep = "/"))
   ...
   simulation.metrics.list <- purrr::map2(iterations, result.path, simulation.metrics)
   GL.metrics.list[[i]] <- Reduce(rbind, simulation.metrics.list)
   
}
GL.metrics.list.df <- purrr::reduce(GL.metrics.list, dplyr::full_join)
save(GL.metrics.list.df, file = paste("NemaScan_Performance_GenomicLocations",today,"RData", sep = "."))
